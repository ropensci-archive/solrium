...
3. Navigate in your terminal to an example Solr database: `cd /usr/local/Cellar/solr/4.5.1/example/`
4. Start solr: `java -jar start.jar`
5. Navigate in your browser to [http://localhost:8983/solr/#/](http://localhost:8983/solr/#/)
6. Navigate in your terminal to `cd /usr/local/Cellar/solr/4.5.1/example/exampledocs/`, then execute `./post.sh vidcard.xml` in that directory. This inserts some data into the example database.
...
Local setup of Solr on Linux using Linuxbrew
======
...
Local setup of Solr on Windows
======

I'm a Mac user, so I'm not too familiar with Windows, but will get to this soon, or send a pull request with instructions in this file.
