...
Development is now following Solr v6 and greater - which introduced many changes, which means many functions here may not work with your Solr installation older than v6.
...
> Currently developing against Solr `v6.6.0`
...
```{r}
(cli <- SolrClient$new(host = 'http://api.plos.org/search', port = NULL))
```
...
```{r eval=FALSE}
SolrClient$new(host = "localhost", port = 8983, errors = "complete")
```
